...
| 补丁 | 作用 | 行数 |
|---|---|---|
| B | API 桥接：12 个函数导出 + `require` 插件 | 1 sed |
| B2 | `runAgentHeadless` 桥接（send_message 依赖） | 1 sed |
| B3 | `listAgentWorkspaces` 桥接 | 1 sed |
| A | MCP 钩子：注入 `session` MCP server | 1 sed |
| C1-5 | 频道+模型+API Key 元数据覆盖 | 5 sed |
| D+E | Renderer 同步 + hydration 幂等守卫移除 | 复制+sed |
...
main.cjs
  ...
  ├─ 补丁 C: 频道/模型元数据覆盖
  └─ proma-dev-patches.cjs（710+ 行）
       ├─ createToolHandlers(sourceSessionId) → 10 个 handler
       ├─ createSessionMcpServer(sdk, z, sourceSessionId) → 内部 Agent
       ├─ createExternalHttpBridge() → localhost HTTP server
       └─ global.__proma_getMcpServers__ 注册

proma-mcp-server.cjs（230+ 行）
...
- **Handler 共享**：内部 MCP server 和外部 HTTP bridge 共用同一套 handler 函数（`createToolHandlers()`）
...
详细 sed 命令见 [完整 Wiki](./.context/proma-dev-wiki.md)。
...
每个步骤都有完整的 Git 提交记录（14 次提交），详见 `workspace-files` 仓库。
