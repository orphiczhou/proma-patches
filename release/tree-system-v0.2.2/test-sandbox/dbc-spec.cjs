 ...
 *
 * 原理: 通过 execFileSync 调用 tree-state.js CLI（不经 shell，数组传参，
 *       规避 Windows cmd 单引号问题）。tree-state.js 的错误码在 stdout
 *       JSON 的 error.code 字段（exit code 1）。
 *
...
const SANDBOX = path.join(__dirname, 'core');       // test-sandbox/core/ — tree-state.js 与 tree 数据同处
const SCRIPT = path.join(SANDBOX, 'tree-state.js'); // 被测脚本副本
...
function run(cmdArgs) {
  // cmdArgs: string[] 不含 'node tree-state.js' 前缀
  try {
    const stdout = execFileSync('node', [SCRIPT, ...cmdArgs], {
      cwd: SANDBOX, encoding: 'utf8', timeout: 20000,
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    let parsed = {};
    try { parsed = JSON.parse(stdout.trim()); } catch (_) {}
    return { ok: !!parsed.ok, error: parsed.error || null, result: parsed, stdout };
  } catch (e) {
    ...
    return {
      ok: false,
      error: parsed.error || { code: 'E_EXEC', msg: (e.message || '').slice(0, 200) },
...
// 期望命令失败且错误码匹配
function expectFail(name, cmdArgs, expectedCode) {
  const r = run(cmdArgs);
...
function expectOk(name, cmdArgs) {
  const r = run(cmdArgs);
...
function setupTreeWithAuditor(budget) {
  const { tid } = setupTree(budget);
  run(['leaf', 'add', tid, '--json', JSON.stringify({
...
function addWorker(tid, leafPath) {
...
CASES.A1 = () => {
  ...
  const { tid } = setupTree();
  const leafId = addWorker(tid, 'A1');
  prepareWorkerForDone(tid, leafId, { expectOutputs: ['missing-deliverable.md'] });
  // 文件不存在 → set-status done 必须被拦
  expectFail('A1 文件缺失拦截', ['leaf', 'set-status', tid, leafId, 'done'], E.DELIVERABLE_MISSING);
};
...
CASES.A2 = () => {
  ...
  const { tid: tid2, auditorSession } = setupTreeWithAuditor();
  const leafId2 = addWorker(tid2, 'A2c');
  run(['event', 'append', tid2, leafId2, '--type', 'brief_echo', '--json', JSON.stringify({ ack: 'ok' })]);
  run(['event', 'append', tid2, leafId2, '--type', 'done', '--json', JSON.stringify({ deliverables: [], self_check: [{ item: 'a2c_setup', pass: true, evidence: 'setup default for strict A5' }] })]);
  expectOk('A2-c 独立 auditor 放行',
...
CASES.A7 = () => {
  ...
  const { tid } = setupTreeWithAuditor();
  const leafId = addWorker(tid, 'A7');
  // 不 append done event，直接 audit pass（用独立 auditor 避免触发 A2）
  expectFail('A7 无 done 事件即 audit pass 拦截',
...
CASES.A3 = () => {
  ...
  const { tid } = setupTree();
  const leafId = addWorker(tid, 'A3');
  // (a) alignment 无 auditor_session_id → 拦
  expectFail('A3-a alignment 无 auditor 拦截',
  ...
  expectFail('A3-b alignment auditor=自己 拦截',
  ...
  const { tid: tid2, auditorSession } = setupTreeWithAuditor();
  const leafId2 = addWorker(tid2, 'A3c');
  expectOk('A3-c alignment+独立auditor 放行',
...
CASES.A4 = () => {
  ...
  const { tid } = setupTree(2); // node_budget=2: root(1) + 1 worker(2) = 满
  addWorker(tid, 'A4a'); // root + A4a = 2 active, 放行
  // 第2个 worker → active_count=2 >= budget=2 → 拦
  expectFail('A4 节点超budget拦截', ['leaf', 'add', tid, '--json', JSON.stringify({
...
CASES.A6 = () => {
  ...
  expectFail('A6 archived前validate拦截', ['leaf', 'set-status', tid, leafId, 'archived'], E.TREE_NOT_VALIDATED);
};
CASES.HARDEN2 = () => {
  console.log('\n[#2] validate 时 done worker 必须有独立 audit_gate (加固点#2)');
  const { tid } = setupTree();
  const leafId = addWorker(tid, 'H2');
  // 篡改: done worker 的 auditor=added_by(自审)，模拟绕过 audit-gate 命令直接改文件
  tamperLeaf(tid, leafId, (l) => {
    l.status = 'done';
    l.audit_gate = { verdict: 'pass', auditor_session_id: UUID.root, ts: '2026-06-23T00:00:00Z' };
  });
  const r = run(['validate', tid]);
...
CASES.V2_FORGED = () => {
  ...
  const { tid } = setupTreeWithAuditor();
  const leafId = addWorker(tid, 'V2');
  // 伪造非树中 UUID → 白名单拦（黑名单下会放行）
  expectFail('V2 伪造非树中UUID auditor 拦截',
...
CASES.V1_RESTORE = () => {
  ...
  expectFail('V1 restore 拒绝伪造backup', ['restore', tid, backupPath], E.TREE_NOT_VALIDATED);
};

// ---------- V3: A1 强制 expect_outputs 非空（堵空交付物，批次4 加固）----------
CASES.V3_EMPTY = () => {
  console.log('\n[V3] done 时 milestone.expect_outputs 必须非空（E_DELIVERABLE_MISSING，堵空交付物）');
  const { tid, auditorSession } = setupTreeWithAuditor();
  const leafId = addWorker(tid, 'V3');
  run(['milestone', 'add', tid, leafId, '--json', JSON.stringify({ id: 'M1', desc: 'empty outputs', expect_outputs: [] })]);
  run(['milestone', 'set-result', tid, leafId, 'M1', '--audit-pass', 'true']);
  run(['event', 'append', tid, leafId, '--type', 'brief_echo', '--json', JSON.stringify({ ack: 'ok' })]);
  run(['event', 'append', tid, leafId, '--type', 'done', '--json', JSON.stringify({ self_check: [{ item: 'v3', pass: true, evidence: 'setup' }] })]);
  run(['audit', 'gate', tid, leafId, '--verdict', 'pass', '--audit-session-id', auditorSession]);
  expectFail('V3 空 expect_outputs 拦截', ['leaf', 'set-status', tid, leafId, 'done'], E.DELIVERABLE_MISSING);
};
...
CASES.T3 = () => {
  ...
  expectFail('T3 第3层 commander 嵌套拦截', ['leaf', 'add', tid, '--json', JSON.stringify({
...
function main() {
  ...
  for (const key of order) {
    ...
    if (typeof CASES[key] === 'function') CASES[key]();
  }
  console.log('\n------------------------------------------------------------');
