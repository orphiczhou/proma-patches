...
> **编写者**: A 叶子任务（draft），树形会话执行体系 retest 验证
> **状态**: 验证通过 ✅
...
| **执行轨** → 子会话 | 写文档、跑代码、长任务、需持久上下文的产出 | `fork_session` + `send_message` | 异步通信、有历史、能跨消息累积 |
...
| 组 | 命令 | 用途 |
|----|------|------|
| **Query** (8) | `leaf get` / `leaf list-active` / `leaf list-all` / `tree dump` | 只读查询 |
| | `drift list` / `heartbeat tail` / `event list` / `validate` | 只读查询 + 一致性校验 |
...
| §4 工作流程 | 6 步流程落到 tree-state.js 子命令级别 |
| §5 tree-state.js 子命令索引 | 列出 21 子命令的用途和调用场景 |
| §6 5 件套契约模板 | Brief / DoD / Report Protocol / Autonomy Boundaries / Self-Audit 完整 YAML |
| §7 事件路由表 | 5 类上行事件的验收/纠偏路由（done→验收 Agent / blocked→决策 / plan→审批 Agent） |
| §8 三档纠偏决策树 | severity=low→nudge / mid→limit / high→prune 的递进执行逻辑 |
| §9 v0.1 待启用能力 | 明确标记心跳/内审/竹节在 v0.1 不要求执行 |
...
| 2 | **5 件套契约模板** | tree-commander SKILL.md §6 | ✅ 5 段 YAML 模板完整 |
...
| A-draft 子会话 | Worker (deepseek-v4-flash) | 本任务 | 🟢 执行中 |
...
| `workspace-files/.context/retest-deliverable/draft-proof.md` | 本验证证明文档 |
